...
from experiments.multi_learner_base import _init_data_folder, make_loop_matrix, \
    calculate_averages, create_graphs, create_img_name, create_param_graph
...
def _calculate_setup_averages(path, models):
...
def _calculate_avg_of_avgs(avgs):
    """Calculates average stats from multiple average stats."""
    steps = len(avgs[0]['max_rewards'])
    avg_of_avgs = {}
    ...
    for key, value in avgs[0].items():
        if type(value) is dict:
            ...
            for sub_key in value.keys():
                avg_of_avgs[key][sub_key] = np.zeros(avgs[0][key][sub_key].shape)
        else:
            avg_of_avgs[key] = np.zeros(avgs[0][key].shape)

    for avg in avgs:
        for key, value in avg.items():
            if type(value) is dict:
                for sub_key, val in value.items():
                    avg_of_avgs[key][sub_key] += val / steps
            else:
                avg_of_avgs[key] += value / steps

    return avg_of_avgs


def calculate_averages2(path, models):
    """For each setup stats of all agents are averaged.
    Returns a list containing average stats from each setup."""
    avgs = []
    for dir in sorted(os.listdir(path)):
        setup_avgs = _calculate_setup_averages(os.path.join(path, dir), models)
        setup_avg = _calculate_avg_of_avgs(setup_avgs)
...
def run_experiment(params, num_of_simulations, num_of_steps,
    ...
    run_avg_stats = calculate_averages2(data_folder, models)
    i = 0
